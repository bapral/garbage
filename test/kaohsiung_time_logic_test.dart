/// - **測試目的**: 高雄市時間過濾邏輯深度測試，驗證 DatabaseService 在處理跨小時邊界（例如 10:55 搜尋 11:05 的點位）時的正確性。
/// - **測試覆蓋**: 
///   - 同小時內的時間過濾（10:10 搜尋 10:00 與 10:15 的點位）。
///   - 跨小時邊界的時間查詢（10:55 搜尋 11:05 的站點）。
///   - 時間偏移邏輯與邊界匹配準確性。
/// - **測試執行順序**: 初始化記憶體資料庫 -> 插入跨越不同時段（含跨小時）的測試站點資料 -> 模擬不同「現在時間」執行查詢 -> 斷言是否能正確跨越小時邊界抓到目標點位。
