/// - **測試目的**: TaipeiGarbageService 深度測試：驗證 CSV 解析細節、時間格式轉換（如 1630 轉 16:30）以及預測車輛的回傳邏輯。
/// - **測試覆蓋**: 
///   - 台北市 CSV 特定時間格式（1630 -> 16:30）解析。
///   - 台北市座標（經緯度）解析準確性。
///   - findTrucksByTime 預測垃圾車的回傳邏輯與顯示名稱（「預定車」）。
/// - **測試執行順序**: 初始化記憶體資料庫 -> 建立臨時目錄並寫入模擬 CSV -> 執行同步流程並驗證存入筆數 -> 執行時間查詢與預測車輛回傳驗證。
