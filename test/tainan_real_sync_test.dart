/// - **測試目的**: 台南市真實同步測試，驗證從真實 API 抓取台南市清運點資料（應超過 10000 筆）並存入資料庫，以及獲取即時 GPS 車輛的穩定性。
/// - **測試覆蓋**: 
///   - 台南市真實清運點資料同步筆數（大於 10000 筆）驗證。
///   - 特定路線（如路線 18）點位獲取。
///   - 即時 GPS API 資料解析（座標與車號有效性）。
/// - **測試執行順序**: 初始化測試環境與臨時目錄 -> 執行 syncDataIfNeeded 並監控同步進度 -> 驗證資料庫總點位數 -> 執行即時 GPS 獲取並斷言車輛資訊有效性。
