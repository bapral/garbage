/// - **測試目的**: 驗證雙北市垃圾車 API 同步邏輯，確保資料更新門檻、同步機制以及異常處理符合預期。
/// - **測試覆蓋**: 
///   - 台北市 API 同步門檻驗證（超過 1100 筆時更新）。
///   - 台北市低於門檻不更新驗證（1000 筆時不更新）。
///   - 新北市 CSV 同步門檻驗證（超過 5000 筆時更新）。
/// - **測試執行順序**: 初始化模擬資料庫與環境 -> 設定測試用的 API 回傳內容（正常與低於門檻） -> 執行同步方法並觸發 API 請求 -> 斷言資料庫中的紀錄筆數。
