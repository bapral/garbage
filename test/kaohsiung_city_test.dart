/// - **測試目的**: 高雄市垃圾車服務邏輯測試，驗證 API 資料解析、資料庫存入流程以及異常連結處理機制。
/// - **測試覆蓋**: 
///   - 模擬 JSON 解析與資料庫同步驗證。
///   - 點位屬性（名稱、抵達時間）準確性檢查。
///   - API 連結異常（如 404 或空回傳）時的穩定性與處理。
/// - **測試執行順序**: 初始化測試環境與模擬 PackageInfo -> 使用 FakeClient 注入模擬 JSON -> 執行高雄市服務的資料同步方法（syncDataIfNeeded） -> 從資料庫中讀取點位並斷言屬性正確性。
