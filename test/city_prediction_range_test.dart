/// - **測試目的**: 驗證各城市垃圾車預測模式的時間範圍邏輯（前 5 分鐘至後 20 分鐘），確保點位在正確的時間視窗內可見。
/// - **測試覆蓋**: 
///   - 25 分鐘可見窗口驗證（前 5 後 20 邏輯）。
///   - 邊界條件測試（早於或晚於 1 分鐘的不可見性）。
///   - 多城市（新北、台北、高雄）預測邏輯一致性驗證。
/// - **測試執行順序**: 初始化記憶體資料庫 -> 插入特定時間的測試垃圾車路線點位 -> 模擬不同「現在時間」並查詢資料庫 -> 斷言查詢結果是否符合時間窗口邊界條件。
